...
func (t *Tracker) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if err := t.client.Get(ctx, req.NamespacedName, wl); err != nil {
		...
		active, err := workloadslicing.FindLatestAdmittedWorkloadForSlice(ctx, t.client, req.Namespace, req.Name, true)
	...
	if workloadslicing.IsElasticWorkload(wl) {
		active, err := workloadslicing.FindLatestAdmittedWorkload(ctx, t.client, wl, true)
		if err != nil {
			return reconcile.Result{}, err
		}
...
func (t *Tracker) updateWorkloadStatusIfNeeded(ctx context.Context, wl *kueue.Workload, pods []*corev1.Pod, current *metav1.Condition, now time.Time) error {
	...
	for _, assignment := range wl.Status.Admission.PodSetAssignments {
		...
		count := int64(ptr.Deref(assignment.Count, specCounts[name]))
		required += count
		scheduled += min(count, activeScheduled[name]+max(succeeded[name], int64(reclaimable[name])))
		succeededCapped += min(count, succeeded[name])
	}
