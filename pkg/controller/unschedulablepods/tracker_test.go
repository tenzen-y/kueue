...
func TestReconcile(t *testing.T) {
	...
	resetCondition := metav1.Condition{
		...
		ObservedGeneration: 2, LastTransitionTime: metav1.NewTime(fakeClock.Now()),
	}

	testCases := map[string]struct {
		features       map[featuregate.Feature]bool
		configuration  *configapi.Configuration // nil uses a positive unschedulable timeout.
		...
		"non-admitted scheduled workload is reset without pods": {
			features: map[featuregate.Feature]bool{
				features.WaitForPodsReadyUnschedulableTimeout: true,
			},
			workloads:      []*kueue.Workload{nonAdmittedWorkload.Clone().Condition(scheduledHistory).Obj()},
			wantConditions: map[string]*metav1.Condition{testWorkload: &resetCondition},
			reconcileTwice: true,
		},
		"absent scheduling history stays absent": {
			...
			workloads:      []*kueue.Workload{nonAdmittedWorkload.Clone().Obj()},
		...
		"unscheduled history is retained": {
			...
			workloads: []*kueue.Workload{nonAdmittedWorkload.Clone().Condition(metav1.Condition{
				Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse,
				Reason: kueue.WorkloadWaitForScheduling, LastTransitionTime: metav1.NewTime(earlier),
			}).Obj()},
			wantConditions: map[string]*metav1.Condition{testWorkload: &metav1.Condition{
				Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse,
				Reason: kueue.WorkloadWaitForScheduling, LastTransitionTime: metav1.NewTime(earlier),
			}},
			reconcileTwice: true,
		...
		"unknown scheduling status is retained": {
			...
			workloads: []*kueue.Workload{nonAdmittedWorkload.Clone().Condition(metav1.Condition{
				Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionUnknown,
				Reason: "Unknown", LastTransitionTime: metav1.NewTime(earlier),
			}).Obj()},
			wantConditions: map[string]*metav1.Condition{testWorkload: &metav1.Condition{
				Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionUnknown,
				Reason: "Unknown", LastTransitionTime: metav1.NewTime(earlier),
		...
		"gate disabled": {
			features: map[featuregate.Feature]bool{
				features.WaitForPodsReadyUnschedulableTimeout: false,
			},
			workloads:      []*kueue.Workload{nonAdmittedWorkload.Clone().Condition(scheduledHistory).Obj()},
			wantConditions: map[string]*metav1.Condition{testWorkload: &scheduledHistory},
			reconcileTwice: true,
		},
		...
		"timeout omitted": {
			...
			configuration:  &configapi.Configuration{WaitForPodsReady: &configapi.WaitForPodsReady{}},
			workloads:      []*kueue.Workload{nonAdmittedWorkload.Clone().Condition(scheduledHistory).Obj()},
			wantConditions: map[string]*metav1.Condition{testWorkload: &scheduledHistory},
			reconcileTwice: true,
		},
		"timeout zero": {
			...
			configuration:  &configapi.Configuration{WaitForPodsReady: &configapi.WaitForPodsReady{UnschedulableTimeout: &metav1.Duration{}}},
			workloads:      []*kueue.Workload{nonAdmittedWorkload.Clone().Condition(scheduledHistory).Obj()},
			wantConditions: map[string]*metav1.Condition{testWorkload: &scheduledHistory},
			reconcileTwice: true,
		},
		"finished workload keeps its history": {
			...
			workloads:      []*kueue.Workload{nonAdmittedWorkload.Clone().Condition(scheduledHistory).FinishedAt(fakeClock.Now()).Obj()},
			wantConditions: map[string]*metav1.Condition{testWorkload: &scheduledHistory},
			reconcileTwice: true,
		},
		"non-admitted variant is not reset": {
			...
			workloads: []*kueue.Workload{nonAdmittedWorkload.Clone().Condition(scheduledHistory).
				ControllerReference(kueue.SchemeGroupVersion.WithKind("Workload"), "parent", "parent-uid").Obj()},
			wantConditions: map[string]*metav1.Condition{testWorkload: &scheduledHistory},
			reconcileTwice: true,
		},
		"gate off does not observe Pods": {
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					UID(testWorkloadUID).
					PodSets(*utiltestingapi.MakePodSet(testPodSetName, 1).Obj()).
		...
		"gate off does not reset a released workload": {
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					UID(testWorkloadUID).
					PodSets(*utiltestingapi.MakePodSet(testPodSetName, 1).Obj()).
					Condition(metav1.Condition{Type: kueue.WorkloadQuotaReserved, Status: metav1.ConditionFalse, Reason: "Pending", LastTransitionTime: metav1.NewTime(fakeClock.Now())}).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse, Reason: kueue.WorkloadWaitForScheduling, LastTransitionTime: metav1.NewTime(earlier)}).
					Obj(),
			},
			pods: []*corev1.Pod{
				testingpod.MakePod("p1", testNamespace).
			...
			wantConditions: map[string]*metav1.Condition{
				testWorkload: &metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse, Reason: kueue.WorkloadWaitForScheduling, LastTransitionTime: metav1.NewTime(earlier)},
			},
		},
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						...
						Obj(), earlier).
					AdmittedAt(true, earlier).
					Condition(metav1.Condition{Type: kueue.WorkloadEvicted, Status: metav1.ConditionTrue, Reason: kueue.WorkloadEvictedByPodsReadyTimeout, Message: "Exceeded the PodsReady timeout", LastTransitionTime: metav1.NewTime(fakeClock.Now())}).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionTrue, Reason: kueue.WorkloadAllRequiredPodsScheduled, Message: allPodsScheduledMessage, LastTransitionTime: metav1.NewTime(fakeClock.Now().Add(-30 * time.Second))}).
					Obj(),
			},
			pods: []*corev1.Pod{
				testingpod.MakePod("p1", testNamespace).
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						...
						Obj(), earlier).
					AdmittedAt(true, earlier).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionTrue, Reason: kueue.WorkloadAllRequiredPodsScheduled, Message: allPodsScheduledMessage, LastTransitionTime: metav1.NewTime(muchEarlier)}).
					Obj(),
			},
			wantConditions: map[string]*metav1.Condition{
				testWorkload: &metav1.Condition{
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						...
						Obj(), earlier).
					AdmittedAt(true, earlier).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse, Reason: kueue.WorkloadWaitForStart, Message: workload.PodsNotReadyMessage, LastTransitionTime: metav1.NewTime(fakeClock.Now().Add(-30 * time.Second))}).
					Obj(),
			},
			pods: []*corev1.Pod{
				testingpod.MakePod("p1", testNamespace).
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						...
						Obj(), earlier).
					AdmittedAt(true, earlier).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse, Reason: kueue.WorkloadWaitForScheduling, Message: unscheduledPodsMessage, LastTransitionTime: metav1.NewTime(earlier)}).
					Obj(),
			},
			pods: []*corev1.Pod{
				testingpod.MakePod("p1", testNamespace).
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						...
						Obj(), earlier).
					AdmittedAt(true, earlier).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionTrue, Reason: kueue.WorkloadAllRequiredPodsScheduled, Message: allPodsScheduledMessage, LastTransitionTime: metav1.NewTime(earlier)}).
					Obj(),
			},
			pods: []*corev1.Pod{
				testingpod.MakePod("p1", testNamespace).
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						...
						Obj(), earlier).
					AdmittedAt(true, earlier).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse, Reason: kueue.WorkloadWaitForScheduling, Message: unscheduledPodsMessage, LastTransitionTime: metav1.NewTime(fakeClock.Now().Add(-30 * time.Second))}).
					Obj(),
			},
			wantConditions: map[string]*metav1.Condition{
				testWorkload: &metav1.Condition{
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						...
						Obj(), earlier).
					AdmittedAt(true, earlier).
					ReclaimablePods(kueue.ReclaimablePod{Name: testPodSetName, Count: 2}).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse, Reason: kueue.WorkloadWaitForScheduling, Message: unscheduledPodsMessage, LastTransitionTime: metav1.NewTime(fakeClock.Now().Add(-30 * time.Second))}).
					Obj(),
			},
			wantConditions: map[string]*metav1.Condition{
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						...
						Obj(), earlier).
					AdmittedAt(true, earlier).
					ReclaimablePods(kueue.ReclaimablePod{Name: testPodSetName, Count: 1}).
					Obj(),
			},
			pods: []*corev1.Pod{
				testingpod.MakePod("p1", testNamespace).
		...
		"surplus succeeded pods in one podset do not fill another podset's grant": {
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					UID(testWorkloadUID).
					PodSets(*utiltestingapi.MakePodSet("leader", 1).
						Request(corev1.ResourceCPU, "1").
						Obj(), *utiltestingapi.MakePodSet("worker", 1).
						Request(corev1.ResourceCPU, "1").
						Obj()).
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						PodSets(utiltestingapi.MakePodSetAssignment("leader").
							Count(1).
							Obj(), utiltestingapi.MakePodSetAssignment("worker").
							Count(1).
							Obj()).
						Obj(), earlier).
					AdmittedAt(true, earlier).
		...
		"pods of another podset do not satisfy the grant": {
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					PodSets(*utiltestingapi.MakePodSet("leader", 1).
						Request(corev1.ResourceCPU, "1").
						Obj(), *utiltestingapi.MakePodSet("worker", 2).
						Request(corev1.ResourceCPU, "1").
						Obj()).
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						PodSets(utiltestingapi.MakePodSetAssignment("leader").
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					Condition(metav1.Condition{
						...
						Reason: "Admitted", LastTransitionTime: metav1.NewTime(earlier),
					}).
					Obj(),
			},
			pods: []*corev1.Pod{
		...
		"scale-down before the first observation still requires the admission count": {
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					PodSets(*utiltestingapi.MakePodSet(testPodSetName, 1).Obj()).
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						PodSets(utiltestingapi.MakePodSetAssignment(testPodSetName).Count(2).Obj()).
						Obj(), earlier).
					AdmittedAt(true, earlier).
					Obj(),
			},
			...
			wantConditions: map[string]*metav1.Condition{
				testWorkload: {
					Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse,
					Reason: kueue.WorkloadWaitForScheduling, Message: unscheduledPodsMessage,
					LastTransitionTime: metav1.NewTime(fakeClock.Now()),
				},
			},
		},
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					Condition(metav1.Condition{
						Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionFalse,
						Reason: kueue.WorkloadWaitForScheduling, Message: unscheduledPodsMessage,
						LastTransitionTime: metav1.NewTime(fakeClock.Now().Add(-30 * time.Second)), ObservedGeneration: 1,
					}).
					Obj(),
			},
			pods: []*corev1.Pod{
		...
		"missing admission count falls back to the spec count": {
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					PodSets(*utiltestingapi.MakePodSet(testPodSetName, 2).Obj()).
					ReserveQuotaAt(utiltestingapi.MakeAdmission("cq").
						PodSets(kueue.PodSetAssignment{Name: testPodSetName}).
						Obj(), earlier).
					AdmittedAt(true, earlier).
					Obj(),
			},
		...
		"elastic scale-down preserves the scheduled observation even with a pending replacement": {
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload(testWorkload, testNamespace).
					...
					AdmittedAt(true, earlier).
					Condition(metav1.Condition{
						Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionTrue,
						Reason: kueue.WorkloadAllRequiredPodsScheduled, Message: allPodsScheduledMessage,
						LastTransitionTime: metav1.NewTime(fakeClock.Now().Add(-30 * time.Second)), ObservedGeneration: 1,
					}).
					Obj(),
			},
		...
		"elastic job: the replaced slice still admitted redirects to the replacement": {
			...
			workloads: []*kueue.Workload{
				utiltestingapi.MakeWorkload("wl-1", testNamespace).
					...
					Annotation(workloadslicing.EnabledAnnotationKey, workloadslicing.EnabledAnnotationValue).
					Condition(metav1.Condition{Type: kueue.WorkloadPodsScheduled, Status: metav1.ConditionTrue, Reason: kueue.WorkloadAllRequiredPodsScheduled, Message: allPodsScheduledMessage, LastTransitionTime: metav1.NewTime(muchEarlier.Add(30 * time.Second))}).
					Obj(),
				utiltestingapi.MakeWorkload("wl-2", testNamespace).
					UID(testWorkloadUID).
					PodSets(*utiltestingapi.MakePodSet(testPodSetName, 2).
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			wantStatuses := make(map[string]*kueue.WorkloadStatus, len(tc.workloads))
			...
			for _, wl := range tc.workloads {
				...
				storedWorkload := &kueue.Workload{}
				if err := kClient.Get(ctx, client.ObjectKeyFromObject(wl), storedWorkload); err != nil {
					t.Fatalf("Could not get initial workload %s: %v", wl.Name, err)
				}
				wantStatuses[wl.Name] = storedWorkload.Status.DeepCopy()
			}
			...
			waitForPodsReady := &configapi.WaitForPodsReady{UnschedulableTimeout: &metav1.Duration{Duration: time.Minute}}
			...
			for wlName, wantCondition := range tc.wantConditions {
				...
				wantStatus := wantStatuses[wlName]
				apimeta.RemoveStatusCondition(&wantStatus.Conditions, kueue.WorkloadPodsScheduled)
				if wantCondition != nil {
					apimeta.SetStatusCondition(&wantStatus.Conditions, *wantCondition)
				}
	...
	testCases := map[string]struct {
		...
		"create of a linked pod enqueues its workload": {
			pod: testingpod.MakePod("p1", testNamespace).
				...
				Label(constants.PodSetLabel, string(testPodSetName)).
				Obj(),
			wantRequests: []reconcile.Request{{NamespacedName: types.NamespacedName{Namespace: testNamespace, Name: testWorkload}}},
		},
		...
		"the workload slice name annotation takes precedence over the workload annotation": {
			pod: testingpod.MakePod("p1", testNamespace).
				...
				Label(constants.PodSetLabel, string(testPodSetName)).
				Obj(),
			wantRequests: []reconcile.Request{{NamespacedName: types.NamespacedName{Namespace: testNamespace, Name: "wl-1"}}},
		},
	...
	testCases := map[string]struct {
		...
		"update relinking the pod enqueues both workloads": {
			...
			newPod: testingpod.MakePod("p1", testNamespace).
				...
				Obj(),
			wantRequests: []reconcile.Request{
				{NamespacedName: types.NamespacedName{Namespace: testNamespace, Name: "wl-a"}},
				{NamespacedName: types.NamespacedName{Namespace: testNamespace, Name: "wl-b"}},
			},
		},
		"update unlinking the pod enqueues the previous workload": {
			oldPod: testingpod.MakePod("p1", testNamespace).
				...
				Label(constants.PodSetLabel, string(testPodSetName)).
				Obj(),
			newPod:       testingpod.MakePod("p1", testNamespace).Obj(),
			wantRequests: []reconcile.Request{{NamespacedName: types.NamespacedName{Namespace: testNamespace, Name: "wl-a"}}},
		},
		"update linking the pod enqueues the new workload": {
			oldPod: testingpod.MakePod("p1", testNamespace).Obj(),
			newPod: testingpod.MakePod("p1", testNamespace).
				Annotation(kueue.WorkloadAnnotation, testWorkload).
				Label(constants.PodSetLabel, string(testPodSetName)).
				Obj(),
...
func TestTracker_Create(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"create of a pending workload": {
			workload: utiltestingapi.MakeWorkload(testWorkload, testNamespace).Obj(),
		},
...
func TestTracker_Update(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"update to an admitted workload": {
			oldWorkload: utiltestingapi.MakeWorkload(testWorkload, testNamespace).Obj(),
