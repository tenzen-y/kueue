	...
	testCases := map[string]struct {
		...
		"gate off ignores an expired scheduling deadline from a previous configuration": {
			...
			workload: *utiltestingapi.MakeWorkload("wl", "ns").
				...
				Obj(),
			waitForPodsReady:    &waitForPodsReadyConfig{timeout: 5 * time.Minute, unschedulableTimeout: new(time.Second)},
		...
		"PodsReady=False/WaitForScheduling with a delayed observation; counting unschedulableTimeout since admission": {
			featureGates: map[featuregate.Feature]bool{
				features.WaitForPodsReadyUnschedulableTimeout: true,
			},
			workload: kueue.Workload{
		...
		"PodsReady=False/WaitForScheduling with a current PodsScheduled=False; the scheduling deadline is capped at admitted.LastTransitionTime + timeout": {
			...
			waitForPodsReady:    &waitForPodsReadyConfig{timeout: 5 * time.Minute, unschedulableTimeout: new(6 * time.Minute)},
			wantUnderlyingCause: kueue.WorkloadWaitForScheduling,
