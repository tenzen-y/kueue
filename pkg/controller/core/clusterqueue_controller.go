...
func (r *ClusterQueueReconciler) NotifyTopologyUpdate(oldTopology, newTopology *kueuealpha.Topology) {
	...
	r.nonCQObjectUpdateCh <- event.TypedGenericEvent[iter.Seq[kueue.ClusterQueueReference]]{
		Object: r.cache.ClusterQueuesUsingTopology(kueue.TopologyReference(topology.Name)),
	}
}
...
func (r *ClusterQueueReconciler) NotifyWorkloadUpdate(oldWl, newWl *kueue.Workload) {
	...
	switch {
	case oldWl != nil && newWl != nil && oldWl.Spec.QueueName != newWl.Spec.QueueName:
		// Update Event
		wls = []*kueue.Workload{oldWl, newWl}
	case oldWl == nil:
		// Create Event.
...
func (r *ClusterQueueReconciler) NotifyResourceFlavorUpdate(oldRF, newRF *kueue.ResourceFlavor) {
	...
	r.nonCQObjectUpdateCh <- event.TypedGenericEvent[iter.Seq[kueue.ClusterQueueReference]]{
		Object: func(yield func(kueue.ClusterQueueReference) bool) {
			for _, cqName := range r.cache.ClusterQueuesUsingFlavor(kueue.ResourceFlavorReference(rfName)) {
				if !yield(cqName) {
					return
				}
			}
...
func (r *ClusterQueueReconciler) NotifyAdmissionCheckUpdate(oldAc, newAc *kueue.AdmissionCheck) {
	...
	r.nonCQObjectUpdateCh <- event.TypedGenericEvent[iter.Seq[kueue.ClusterQueueReference]]{
		Object: func(yield func(kueue.ClusterQueueReference) bool) {
			for _, cq := range r.cache.ClusterQueuesUsingAdmissionCheck(acName) {
				if !yield(cq) {
					return
				}
			}
