...
func TestReconcileGenericJob(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"waitForPodsReady with scheduling tracking adds the workload annotation": {
			...
			reconcilerOptions: []Option{WithWaitForPodsReady(&configapi.WaitForPodsReady{UnschedulableTimeout: &metav1.Duration{Duration: time.Minute}})},
			req:               baseReq,
	...
	cases := map[string]struct {
		...
		"all options are passed": {
			inputOpts: []Option{
				...
				WithWaitForPodsReady(&configapi.WaitForPodsReady{UnschedulableTimeout: &metav1.Duration{Duration: time.Minute}}),
			...
			wantOpts: Options{
				...
				WaitForPodsReadyConfig:     &configapi.WaitForPodsReady{UnschedulableTimeout: &metav1.Duration{Duration: time.Minute}},
...
func TestReconcileGenericJobWithWaitForPodsReady(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"non-admitted workload resets readiness without changing scheduling": {
			features:         map[featuregate.Feature]bool{features.WaitForPodsReadyUnschedulableTimeout: true},
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			configuration := ptr.Deref(tc.configuration, configapi.WaitForPodsReady{UnschedulableTimeout: &metav1.Duration{Duration: time.Minute}})
			options := []Option{
				WithClock(fakeClock),
