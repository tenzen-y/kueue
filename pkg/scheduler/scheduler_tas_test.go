...
func TestScheduleForTAS(t *testing.T) {
	...
	cases := map[string]struct {
		...
		// must land on distinct nodes. Reuses defaultSingleLevelTopology and the
		// defaultTASFlavor shape (NodeLabel("tas-node","true")+TopologyName("tas-single-level"))
		// to avoid duplicating fixture scaffolding.
...
func TestScheduleForTASPreemption(t *testing.T) {
	...
	cases := map[string]struct {
		...
		// W3 in flavor-a wants 2 CPU. With Diff 1/2's shared-leaf aliasing, the
		// preemptor's view of x1 correctly shows 4 CPU used so the admission path
		// selects Preempt mode; classical preemption then picks W1 as the same-
		// flavor lower-priority candidate, simulated removal drops tasUsage by 1
		// CPU (W3 now fits), and W1 is preempted. Without the aliasing, flavor-a's
