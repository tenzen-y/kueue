...
var _ = ginkgo.Describe("Job controller", ginkgo.Label("job:jax", "area:jobs"), ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("Should reconcile JAXJobs", func() {
		kfJob := kubeflowjob.KubeflowJob{KFJobControl: (*workloadjaxjob.JobControl)(testingjaxjob.MakeJAXJob(jobName, ns.Name).JAXReplicaSpecsDefault().Obj())}
		...
		kftesting.ShouldReconcileJob(ctx, k8sClient, kfJob, createdJob, []kftesting.PodSetsResource{
			{
				RoleName:    kftraining.JAXJobReplicaTypeWorker,
				ResourceCPU: "spot",
			},
		})
	})
	...
	ginkgo.It("Should not manage a job without a queue-name submitted to an unmanaged namespace", func() {
		...
		kfJob := kubeflowjob.KubeflowJob{KFJobControl: (*workloadjaxjob.JobControl)(testingjaxjob.MakeJAXJob(jobName, "unmanaged-ns").Suspend(false).Obj())}
		createdJob := kubeflowjob.KubeflowJob{KFJobControl: (*workloadjaxjob.JobControl)(&kftraining.JAXJob{})}
...
var _ = ginkgo.Describe("Job controller for workloads when only jobs with queue are managed", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("Should reconcile jobs only when queue is set", func() {
		...
		job := testingjaxjob.MakeJAXJob(jobName, ns.Name).
			JAXReplicaSpecsDefault().
			Obj()
		util.MustCreate(ctx, k8sClient, job)
		lookupKey := types.NamespacedName{Name: jobName, Namespace: ns.Name}
		createdJob := &kftraining.JAXJob{}
		gomega.Expect(k8sClient.Get(ctx, lookupKey, createdJob)).Should(gomega.Succeed())

		createdWorkload := &kueue.Workload{}
		wlLookupKey := types.NamespacedName{Name: workloadjaxjob.GetWorkloadNameForJAXJob(job.Name, job.UID), Namespace: ns.Name}
	...
	ginkgo.When("the queue has admission checks", func() {
		...
		ginkgo.It("labels and annotations should be propagated from admission check to job", func() {
			...
			job := testingjaxjob.MakeJAXJob(jobName, ns.Name).
				JAXReplicaSpecsDefault().
				PodAnnotation(kftraining.JAXJobReplicaTypeWorker, "old-ann-key", "old-ann-value").
				PodLabel(kftraining.JAXJobReplicaTypeWorker, "old-label-key", "old-label-value").
			...
			ginkgo.By("verify the PodSetUpdates are propagated to the running job", func() {
				worker := createdJob.Spec.JAXReplicaSpecs[kftraining.JAXJobReplicaTypeWorker].Template
...
var _ = ginkgo.Describe("Job controller when waitForPodsReady enabled", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.DescribeTable("Single job at different stages of progress towards completion",
		...
		ginkgo.Entry("Running JAXJob", kftesting.PodsReadyTestSpec{
		...
		ginkgo.Entry("Running JAXJob; PodsReady=False before", kftesting.PodsReadyTestSpec{
...
var _ = ginkgo.Describe("Job controller interacting with scheduler", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("Should schedule jobs as they fit in their ClusterQueue", func() {
		...
		kfJob := kubeflowjob.KubeflowJob{KFJobControl: (*workloadjaxjob.JobControl)(
			testingjaxjob.MakeJAXJob(jobName, ns.Name).
				JAXReplicaSpecsDefault().
				Queue(localQueue.Name).
				Request(kftraining.JAXJobReplicaTypeWorker, corev1.ResourceCPU, "4").
				Obj(),
		)}
		createdJob := kubeflowjob.KubeflowJob{KFJobControl: (*workloadjaxjob.JobControl)(&kftraining.JAXJob{})}
	...
	ginkgo.When("The workload's admission is removed", func() {
		ginkgo.It("Should restore the original node selectors", func() {
			...
			nodeSelectors := func(j *kftraining.JAXJob) map[kftraining.ReplicaType]map[string]string {
				ret := map[kftraining.ReplicaType]map[string]string{}
				for k := range j.Spec.JAXReplicaSpecs {
					ret[k] = j.Spec.JAXReplicaSpecs[k].Template.Spec.NodeSelector
				}
				return ret
			}
...
var _ = ginkgo.Describe("JAXJob controller with TopologyAwareScheduling", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("should admit workload which fits in a required topology domain", framework.SlowSpec, func() {
		pytorchJob := testingjaxjob.MakeJAXJob("jaxjob", ns.Name).
			...
			Queue(localQueue.Name).
			Request(kftraining.JAXJobReplicaTypeWorker, corev1.ResourceCPU, "100m").
			Obj()
		ginkgo.By("creating a JAXJob", func() {
			util.MustCreate(ctx, k8sClient, pytorchJob)
		})

		wl := &kueue.Workload{}
		wlLookupKey := types.NamespacedName{Name: workloadjaxjob.GetWorkloadNameForJAXJob(pytorchJob.Name, pytorchJob.UID), Namespace: ns.Name}
