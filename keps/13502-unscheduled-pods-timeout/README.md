...
- While `waitForPodsReady` is enabled, the built-in template-based integrations propagate the
  `kueue.x-k8s.io/workload` annotation to the Pod templates of the jobs they start, and the
  pod-based integrations apply it to the gated Pods when those Pods are started, so the
  tracker does not depend on the TopologyAwareScheduling feature gate. Pods created before
  an upgrade may already carry `workload` from TopologyAwareScheduling or
  SchedulerLibraryIntegration and are accepted by name; Pods lacking both the
...
The tracker needs to list the Pods of a Workload without knowing the job type. The existing
`workload` annotation, plus `workload-slice-name` for elastic jobs, serves that purpose. The
built-in template-based integrations propagate the applicable annotations to the Pod templates
through `podset.Merge` when the job is started (and remove them when the job is stopped), and
the pod-based integrations (pod groups, Deployments, StatefulSets and LeaderWorkerSets) apply
them to the gated Pods when those Pods are started:
...
counted against the grant of the active (admitted) slice. This association intentionally
follows the existing Workload-name semantics and does not introduce a separate identifier for
individual Workload incarnations.
...
- Reconciles **Workload** resources (not per Job integration) and watches **Pod**
  ...
  event is relevant when the binding, the phase, the `workload` or `workload-slice-name`
  annotation, the `kueue.x-k8s.io/podset` label, or the presence of a deletion timestamp
  changes. A Workload event is relevant when the Workload is tracked or carries a
...
7. List the Pods linked to the Workload. On a list or index error, return the error (requeue)
   and do **not** patch `PodsScheduled` from failed data.
...
- Pods created by an older Kueue (before the upgrade) may already carry
  `kueue.x-k8s.io/workload` when TopologyAwareScheduling or SchedulerLibraryIntegration was
  enabled; such Pods are accepted by name and observed. Pods lacking both the
  `workload-slice-name` and `workload` indexing annotations are not indexed at all: for a
  Workload whose Pods are all
  ...
  after the upgrade get the `workload` annotation and are tracked; elastic jobs additionally
  get `workload-slice-name`.
...
- On a downgrade, an older version ignores `PodsScheduled` and the `WaitForScheduling`
  reason; a leftover `PodsScheduled` condition stays on the Workload until it is deleted.
...
- Job framework: the initial path maps a current-admission `PodsScheduled=False` /
  ...
  `Started` (and the ready-wait metric) again. The `workload` annotation is injected per the
  gate combinations.
...
- `UnschedulablePodsTracker`: the counting formula (succeeded retained, `reclaimablePods`,
  ...
  requeue, no reset, a new observation after the settle); Pods matched by workload name; the
  Pod and Workload event predicates; the MultiKueue removal (any existing condition,
  regardless of ownership and target, before the lifecycle reset and after the finished
  check); elastic redirection to the active slice, also while both slices are admitted; list
...
- Unscheduled Pod: the Job template gets the `workload` annotation,
...
- One representative operator integration (MPIJob) for Pod discovery: the launcher and
  worker templates get the annotation and the `podset` label, `PodsScheduled` moves from
...
- Whenever `waitForPodsReady` is enabled, the tracker writes a current-admission
  ...
  `kueue.x-k8s.io/workload` annotation. Consumers inspecting Workload conditions, Pod
  templates or Pods must tolerate it.
...
- 2026-09: Design revised: timers driven by the `PodsReady` reason, scheduling deadline
  ...
  lifecycle reset, `PodsReady` reset at the quota release, and enablement tied to
  `waitForPodsReady`.
...
- Central tracking couples scheduling detection with the job framework's readiness path: the
  ...
  Pods).
- `PodsScheduled` / `WaitForScheduling` is visible in the Workload status, and the
  `workload` annotation is added to the Pod templates (or, for the pod-based integrations,
  to the gated Pods) of every job started under `waitForPodsReady`, even when the short
